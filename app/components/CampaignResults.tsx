...
const results = [
  {
    ...
    headline:
      "How a full-funnel Google Ads strategy turned visibility into $1.19M in revenue",
    problem:
      ...
      <div className="mx-auto max-w-6xl">
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-1 flex-col p-7">
                  <span
                    className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-semibold ${accent.badge}`}
                  >
                    {result.theme}
                  </span>

                  <p
                    className={`mt-5 text-[0.7rem] font-bold uppercase tracking-widest ${accent.clientText}`}
                  >
                    {result.client}
                  </p>
                  ...
                  <div className="space-y-3">
                    <div>
                      <p className="text-[0.65rem] font-bold uppercase tracking-wider text-slate-900">
                        Challenge
                      </p>
                  ...
                  <div className="mt-auto pt-6">
                    ...
                    <p
                      className={`mt-1 text-2xl font-bold leading-tight tracking-tight ${accent.resultText}`}
                    >
                      {result.mainResult}
                    </p>
                    ...
                    <div className="mt-4 flex gap-5 border-t border-slate-100 pt-4">
                      {result.metrics.map((metric) => (
                        <div key={metric.label}>
                          <p className={`text-sm font-bold ${accent.metricValue}`}>
                            {metric.value}
                          </p>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-6 py-5 sm:flex-row">
          <p className="text-sm font-medium text-slate-600">
            65+ campaigns managed across Google Ads, Meta Ads, and content —
